    ...
    int height)
{
    ...
    pui->fps = 0.0f;
    pui->fps_start = SDL_GetPerformanceCounter();
    pui->fps_frame_count = 0;
    ...
    PulsarUI *pui)
{
    ...
    double fps_elapsed =
        (double)(fps_now - pui->fps_start) /
        SDL_GetPerformanceFrequency();

    if (fps_elapsed >= 1.0) {

        pui->fps =
            (float)pui->fps_frame_count /
            (float)fps_elapsed;

        pui->fps_frame_count = 0;
        pui->fps_start = fps_now;
    }
