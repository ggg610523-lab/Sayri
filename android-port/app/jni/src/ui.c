...
#ifdef ANDROID
    /*
        Portrait-first mobile scaling.

        The design is treated so that roughly one
        design unit maps to one dp, which makes
        buttons / fonts a comfortable touch size and
        the layout flow down a portrait phone. The
        width (~415dp typical) drives the scale, so
        everything grows together.
    */
    float dpx = (float)width / 415.0f;
    float dpy = (float)height / 900.0f;
    ui->scale = fminf(dpx, dpy);
